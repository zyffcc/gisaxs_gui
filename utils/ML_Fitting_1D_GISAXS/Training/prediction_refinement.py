...
def estimated_function_evaluations(residual_calls: int, n_variables: int) -> int:
    """Estimate optimizer iterations from calls including a 2-point Jacobian."""
    if int(residual_calls) <= 0:
        return 0
    calls_per_evaluation = max(int(n_variables), 0) + 1
    return 1 + (int(residual_calls) - 1) // calls_per_evaluation
...
def refine_candidate(
    ...
    def calculate_residual(x):
        ...
        try:
            ...
            return np.full_like(log_i_exp, 1e6, dtype=np.float64)
        if np.all(np.isfinite(i_fit)):
            return np.log(np.maximum(i_fit, 1e-30)) - log_i_exp
        return np.full_like(log_i_exp, 1e6, dtype=np.float64)

    initial_residual = calculate_residual(x0)
    initial_optimization_log_rmse = float(np.sqrt(np.mean(initial_residual**2)))
    progress = {
        "calls": 0,
        "best_log_rmse": initial_optimization_log_rmse,
    ...
    def residual_fn(x):
        progress["calls"] += 1
        residual = calculate_residual(x)
        log_rmse = float(np.sqrt(np.mean(residual**2)))
        nfev_estimate = estimated_function_evaluations(progress["calls"], len(x0))
        if log_rmse < progress["best_log_rmse"] - refine_stall_tol:
            ...
            progress["last_improve_nfev"] = nfev_estimate
        if progress_interval > 0 and (
            progress["calls"] == 1 or progress["calls"] % int(progress_interval) == 0
        ):
            print(
                f"  refine{progress_label} residual_call={progress['calls']:04d} "
                f"estimated_nfev={nfev_estimate:03d} "
        ...
        if refine_stall_patience > 0 and nfev_estimate - progress["last_improve_nfev"] >= int(
            refine_stall_patience
        ):
            progress["early_stop_reason"] = (
                f"stalled for about {int(refine_stall_patience)} function evaluations; "
    ...
    try:
        try:
            ...
            result_log_rmse = float(np.sqrt(np.mean(calculate_residual(result.x) ** 2)))
            if progress["best_log_rmse"] < result_log_rmse:
                x_final = progress["best_x"]
            else:
                x_final = result.x
                progress["best_log_rmse"] = result_log_rmse
                progress["best_x"] = np.asarray(result.x, dtype=np.float64).copy()
        ...
        return refined, {
            ...
            "estimated_nfev": estimated_function_evaluations(progress["calls"], len(x0)),
