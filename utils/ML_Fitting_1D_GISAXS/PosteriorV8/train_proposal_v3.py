"""Train the V3 bounds-conditioned proposal on immutable V4 solution shards.

This entry point consumes only ``train`` and ``tuning_validation`` rows.  Its
artifacts are explicitly an interpolation pilot, not a production model or a
paper holdout result.
"""
...
from .proposal_training_v3 import (
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    objective = BoundsTrainingObjectiveConfig(
    ...
    config = BoundsProposalTrainingConfig(
        epochs=args.epochs,
