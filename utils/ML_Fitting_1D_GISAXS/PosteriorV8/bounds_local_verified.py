"""Authoritative compatibility gate for bounds-first local inference."""
...
from .bounds_local_inference import BoundsLocalProposalModelPort
from .bounds_local_production import BoundsLocalRawResult, _run_bounds_local_search
...
from .rescue_inference import RescueAudit, RescuePolicy, RetrievalSeed


BOUNDS_LOCAL_VERIFIED_SCHEMA = "gisaxs.posterior_v8.bounds_local_verified_search/v3"
VerifiedSearchStatus = Literal[
...
class BoundsLocalVerifiedResult:
    status: VerifiedSearchStatus
    ...
    raw_generation: BoundsLocalRawResult
    ...
    schema: str = BOUNDS_LOCAL_VERIFIED_SCHEMA
    ...
    def __post_init__(self) -> None:
        ...
        if self.schema != BOUNDS_LOCAL_VERIFIED_SCHEMA:
            raise ValueError("unsupported bounds-local verified schema")
        if self.evaluation_audit_schema != EVALUATION_AUDIT_SCHEMA:
            raise ValueError("unexpected authoritative evaluation schema")
        ...
        if not isinstance(self.raw_generation, BoundsLocalRawResult):
            raise TypeError("raw_generation must be BoundsLocalRawResult")
        ...
        if report is None:
            ...
            if report.audit_schema != EVALUATION_AUDIT_SCHEMA:
                raise ValueError("nested EvaluationReport schema mismatch")
            if tuple(item.candidate_id for item in report.candidates) != tuple(
                ...
                raise ValueError("evaluation report does not cover the raw candidates")
            expected_compatible_ids = tuple(
                item.representative_candidate_id for item in report.parameter_modes
            )
        ...
        if effective_report is None:
            ...
            expected_effective_ids = tuple(
                item.representative_candidate_id for item in effective_report.parameter_modes
            )
        ...
        expected_status: VerifiedSearchStatus
        ...
        total = _integer(
            self.total_forward_evaluations,
...
def run_verified_bounds_local_inference(
    ...
    *,
...
) -> BoundsLocalVerifiedResult:
    """Stop on exact-compatible modes, then assess minimality as metadata."""
    ...
    def compatible_target(values: tuple[CandidateInput, ...], _raw_forward_used: int) -> bool:
        report = current_report(values)
        return report is not None and len(report.parameter_modes) >= target

    raw = _run_bounds_local_search(
        curve_inputs,
        curve,
        model=model,
        ...
        seed=seed,
        refiner=refiner,
        stop_when=compatible_target,
    )
    report = current_report(raw.candidates)
    compatible_ids = (
        ()
        if report is None
        else tuple(item.representative_candidate_id for item in report.parameter_modes)
    )
    ...
    effective_ids = (
        ()
        if effective_report is None
        else tuple(item.representative_candidate_id for item in effective_report.parameter_modes)
    )
    if len(compatible_ids) >= target:
        status: VerifiedSearchStatus = "compatible_target_reached"
    elif compatible_ids:
    ...
    return BoundsLocalVerifiedResult(
        status=status,
...
__all__ = [
    "BOUNDS_LOCAL_VERIFIED_SCHEMA",
    "BoundsLocalVerifiedResult",
    "VerifiedSearchStatus",
