"""CLI for explicit local-target Posterior V8 proposal training.

The current Phase-2 NPZ shards use truth-centered global-coordinate boxes and
are refused by default.  ``--allow-truth-centered-range-pilot`` is therefore
an explicit engineering-comparison switch, not a production-data approval.

Invoke as ``python -m utils.ML_Fitting_1D_GISAXS.PosteriorV8.train_proposal_v2``.
"""
...
from .proposal_training_v2 import LocalProposalTrainingConfig, train_local_proposal
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Permit legacy Phase-2 truth-centered boxes for engineering comparison only; "
            "the manifest will record that non-production scientific scope."
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    objective = LocalTrainingObjectiveConfig(
    ...
    config = LocalProposalTrainingConfig(
        epochs=args.epochs,
        ...
        objective=objective,
        allow_truth_centered_range_pilot=args.allow_truth_centered_range_pilot,
    )
