...
from utils.ai_fitting_profiles import FittingProfile, profile_registry
...
class FittingPipeline:
    ...
    def __init__(self, script_path: Path | None = None) -> None:
        self.script_path = script_path or (
            Path(__file__).resolve().parent / "ML_Fitting_1D_GISAXS" / "Training" / "predict_topk.py"
        )
    ...
    def build_args(self, request: FittingRequest) -> list[str]:
        ...
        args = [
            ...
            "--num_samples", str(profile.candidate_count),
            "--top_k", str(profile.top_k),
            "--sampling_std", str(profile.sampling_std),
            "--sampling_scales", ",".join(str(scale) for scale in profile.sampling_scales),
            "--refine_top_n", str(profile.refinement_count),
            "--refine_max_nfev", str(profile.max_evaluations),
            "--refine_q_stride", str(profile.q_stride),
            "--refine_ftol", str(profile.tolerance),
            "--refine_xtol", str(profile.tolerance),
            "--refine_gtol", str(profile.tolerance),
            "--refine_target_logrmse", str(profile.target_log_rmse),
            "--refine_stall_patience", str(profile.stall_patience),
            "--refine_stall_tol", str(profile.stall_tolerance),
            "--complexity_penalty", str(profile.complexity_penalty),
            "--parameter_mode_radius", str(profile.parameter_mode_radius),
            "--progress_interval", str(profile.progress_interval),
            "--seed", str(profile.random_seed),
            "--score_mode", "hybrid_log_relative",
            "--rank_mode", "physics",
    ...
    @staticmethod
    def summarize(output_dir: Path, profile: FittingProfile, runtime_seconds: float, exit_code: int, cancelled: bool = False) -> FittingRunSummary:
    ...
    def run(
        ...
        summary = self.summarize(request.output_dir, request.profile, time.perf_counter() - started, exit_code)
