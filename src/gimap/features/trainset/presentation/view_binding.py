"""Feature-owned binding between the Trainset PyQt page and its ViewModel."""
...
class TrainsetViewBinding(QObject):
    ...
    def initialize(self) -> None:
        ...
        if isinstance(remembered, dict) and int(remembered.get("schema_version", 0)) >= 2:
            self.config = self.trainset_view_model.merge_config_with_defaults(
                remembered
            )
    ...
    def _collect_config(self) -> Dict[str, Any]:
        ...
        particle = next((spec for spec in self.catalog.plugins("particle") if spec.label == particle_label), None)
        ...
        interference = next((spec for spec in self.catalog.plugins("interference") if spec.label == interference_label), None)
    ...
    def _apply_config_to_page(self, config: Dict[str, Any]) -> None:
        ...
        special = {
            "sample.particle_label": next((spec.label for spec in self.catalog.plugins("particle") if spec.key == config["sample"]["particles"][0]["plugin"]), "Spherical segment"),
            "sample.particle_material": config["sample"]["particles"][0].get("material", "Copper"),
            "sample.interference_label": next((spec.label for spec in self.catalog.plugins("interference") if spec.key == config["sample"]["interference"].get("plugin")), "None"),
        }
        for path, widget in self.page.fields.items():
            if path in special:
        ...
        interference_plugin = self.catalog.plugin("interference", interference_config.get("plugin", "none"))
    ...
    def _particle_plugin_changed(self, label: str) -> None:
        plugin = next((spec for spec in self.catalog.plugins("particle") if spec.label == label), None)
        if plugin is None:
            return
    ...
    def _interference_plugin_changed(self, label: str) -> None:
        plugin = next((spec for spec in self.catalog.plugins("interference") if spec.label == label), None)
        if plugin is None:
            return
    ...
    def _update_geometry_label(self) -> None:
        try:
            ...
            if ranges is None:
                raise RuntimeError(
                    self.trainset_view_model.state.error_message
                    or "Geometry calculation failed"
                )
    ...
    def _new_random_mask_example(self) -> None:
        try:
            ...
            self._random_mask_example = self.trainset_view_model.generate_random_mask(
                shape, config
            )
            if self._random_mask_example is None:
                raise RuntimeError(
                    self.trainset_view_model.state.error_message
                    or "Random mask generation failed"
                )
    ...
    def _impact_range(self, config: Dict[str, Any]) -> tuple[str, str, float, float]:
        ...
        if plugin == "physical_background":
            ...
            definition = next(item for item in self.catalog.background_parameters() if item["key"] == key)
    ...
    def _compute_what_if(
        ...
        if result is None:
            raise RuntimeError(
                self.trainset_view_model.state.error_message
                or "Manual trainset simulation failed"
            )
        return result
    ...
    def _prepare_job(self, local: bool) -> None:
        ...
        try:
            ...
            if self.package_dir is None:
                raise RuntimeError(
                    self.trainset_view_model.state.error_message
                    or "Failed to prepare job package"
                )
    ...
    def _start_local_process(self, arguments, follow_up=None) -> None:
        ...
        if not started:
            QMessageBox.warning(
                ...
                "Local backend",
                self.trainset_view_model.state.error_message
                or "Failed to start local process",
            )
            return
    ...
    def _submit_maxwell(self) -> None:
        ...
        try:
            ...
            self._run_worker(
                lambda: self.trainset_view_model.submit_remote_job(
                    config, self.package_dir
                ),
    ...
    def _refresh_job(self) -> None:
        ...
        try:
            ...
            self._run_worker(
                lambda: self.trainset_view_model.query_remote_job(
                    config, job_id
                ),
                self._job_refreshed,
                "Job refresh failed",
    ...
    def _sync_results(self) -> None:
        try:
            ...
            self._run_worker(
                lambda: self.trainset_view_model.download_remote_results(
                    config, destination
                ),
                lambda _result: (
                    self._load_local_metrics(),
