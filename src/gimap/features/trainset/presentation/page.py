...
class TrainsetBuildPage(QWidget, TrainsetPageView):
    ...
    def _bind_shell(self) -> None:
        ...
        self.back_button.clicked.connect(
            lambda: self.step_list.setCurrentRow(
                max(0, self.step_list.currentRow() - 1)
            )
        )
    ...
    def _dataset_page(self) -> QWidget:
        ...
        particle_labels = [
            item.label for item in self.catalog.plugins("particle")
        ]
        ...
        interference_labels = [
            item.label for item in self.catalog.plugins("interference")
        ]
    ...
    def _preview_page(self) -> QWidget:
        ...
        for group_name, keys in background_groups:
            definitions = [
                item
                for item in self.catalog.background_parameters()
                if item["key"] in keys
            ]
