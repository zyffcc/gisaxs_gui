...
class Detector:
    ...
    def _get_parameter_hash(self):
        ...
        params_str = (
            f"{self.detector}_{self.beam_center}_{self.distance}_{self.theta_in}_{self.wavelength}"
        )
    ...
    def calculate_q_vectors(self, force_recalculate=False):
        ...
        if (
            ...
            and self._q_cache is not None
            and self._q_cache_hash == current_hash
        ):
...
def create_detector_from_image_and_params(
    image_shape,
    pixel_size_x,
    pixel_size_y,
    beam_center_x,
    ...
    beam_center = [beam_center_x * pixel_size_x_mm, beam_center_y * pixel_size_y_mm]
...
class Mask:
    ...
    def mask_center(self, radius=5, bar="False"):
        ...
        beam_center_pixel = [
            int(self.beam_center[0] / self.detector[1] * self.detector[0]),
    ...
    def mask_reflect(self, incident_angle, radius=5, bar="False"):
        ...
        beam_center_pixel_re = [
            int(self.beam_center[0] / self.detector[1] * self.detector[0]),
            self.detector[2]
            - int(
                self.beam_center[1] / self.detector[3] * self.detector[2]
                + self.distance * 2 * np.tan(incident_angle)
            ),
        ]
        mask = np.ones((self.detector[2], self.detector[0]))
        for i in range(mask.shape[0]):
            for j in range(mask.shape[1]):
                if (i - beam_center_pixel_re[1]) ** 2 + (
                    j - beam_center_pixel_re[0]
                ) ** 2 < radius**2:
                    mask[i, j] = 0
        if bar == "True":
            bar_para = np.array(
                [
                    [
                        beam_center_pixel_re[1],
                        beam_center_pixel_re[0],
                        2.5,
                        np.deg2rad(np.random.randint(90, 270)),
                    ]
                ]
            )
            cos_angle = np.cos(bar_para[:, 3])
    ...
    def mask_gap(self, v_number, h_number, gap_size, model="random"):
        ...
        if model == "random":
            ...
            v_number = v_number + 1
            position_v = np.linspace(gap_size, self.detector[0], v_number, endpoint=False).astype(
                int
            )
    ...
    def add_mask(self, prob=0.5):
        ...
        if np.random.rand() < prob:
            mask &= self.mask_gap(
                np.random.randint(1, 4), 1, gap_size=np.random.randint(3, 5), model="uniform"
            )
