...
def cylinder_form_factor_pd(q, R, sigma_R, h, sigma_h, n_R=13, n_h=13, nsig=4.0, n_orient=24):
    """
    ...
    *,
    ...
    def f(q, *params):
        if len(params) != len(template):
            raise ValueError(
                f"参数数量不匹配。需要 {len(template)} 个：{template}，实际 {len(params)} 个。"
            )
        ...
        for i, shape in enumerate(spec, 1):
            if shape == "sphere":
                ...
                sD = params[idx + 4]
                idx += 5
                P = sphere_form_factor_pd(
                    q_arr, R, sR, n_samples=int(pd_defaults["n_samples"]), nsig=pd_defaults["nsig"]
                )
                ...
                P = cylinder_form_factor_pd(
                    ...
                    h,
                    sh,
                    n_R=int(cyl_defaults["n_R"]),
                    n_h=int(cyl_defaults["n_h"]),
                    nsig=cyl_defaults["nsig"],
                    n_orient=int(cyl_defaults["n_orient"]),
                )
        ...
        BG = params[idx]
        sigma_Res = params[idx + 1]
        nu_Res = params[idx + 2]
        int_Res = params[idx + 3]
        k = params[idx + 4]
    ...
    *,
    ...
    for i, shape in enumerate(spec, 1):
        if shape == "sphere":
            ...
            S = (
                np.ones_like(q_arr, dtype=float)
                if (D == 0 or sD == 0)
                else structure_factor_1d(q_arr, D, sD)
            )
    ...
    # 取出分辨率与缩放因子
    BG = float(params[idx])
    sigma_Res = float(params[idx + 1])
    nu_Res = float(params[idx + 2])
    int_Res = float(params[idx + 3])
    k = float(params[idx + 4])
