...
def _contract_from_metadata(
    manifest: Mapping[str, Any], config: Mapping[str, Any], metadata: Mapping[str, Any]
) -> ModelContract:
    return ModelContract(
        version=str(_first_known(manifest.get("version"), config.get("version"), "legacy")),
        architecture=str(
            _first_known(
                manifest.get("architecture"), config.get("architecture"), "ML1DGISAXSSlotModel"
            )
        ),
        max_points=int(
            _first_known(
                manifest.get("max_points"),
        ...
        max_slots=int(
            _first_known(
                manifest.get("max_slots"), config.get("max_slots"), metadata.get("max_slots"), 4
            )
        ),
        num_types=int(
            _first_known(
                manifest.get("num_types"), config.get("num_types"), metadata.get("num_types"), 4
            )
        ),
...
def _model_info_for_artifact(model_dir: Path, artifact_path: Path, artifact_type: str) -> ModelInfo:
    ...
    metadata = _read_json(model_dir / "dataset_metadata.json") or _read_json(
    ...
    for base_dir in base_dirs:
        ...
        candidates = (
            ...
            if base.is_file()
                ...
                base,
                *[p for p in sorted(base.iterdir()) if p.is_dir() or p.suffix.lower() == ".keras"],
            ]
        )
        for candidate in candidates:
            for info in discover_model_in_path(candidate):
                key = (
                    info.artifact_path.resolve()
                    if info.artifact_path.exists()
                    else info.artifact_path.absolute()
                )
                if key in seen:
                    continue
                seen.add(key)
...
def validate_loaded_model(model: Any, info: ModelInfo) -> None:
    ...
    if required_inputs:
        actual_inputs = set(getattr(model, "input_names", ()) or ()) or _tensor_names(
            getattr(model, "inputs", ())
        )
    ...
    if required_outputs:
        actual_outputs = set(getattr(model, "output_names", ()) or ()) or _tensor_names(
            getattr(model, "outputs", ())
        )
