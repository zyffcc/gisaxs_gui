...
from .views import IndependentFitWindowView, IndependentImageWindowView
...
class InsituBatchImageLoader(QThread):
    ...
    def __init__(
        ...
        self.fitting_view_model = (
            fitting_view_model or _create_default_fitting_view_model()
        )
        ...
        try:
            ...
            for index, path in enumerate(self.file_paths):
                ...
                if self.mirror_fill_gaps:
                    ...
                    data = _scientific_commands(self).image.mirror_gaps(
                        data,
...
class InsituCutWorker(QThread):
    ...
    def __init__(self, payload: dict, compute_cut=None):
        ...
        self.compute_cut = (
            compute_cut
            or _create_default_fitting_view_model().science.insitu_cut
        )
...
class IndependentMatplotlibWindow(QMainWindow, IndependentImageWindowView):
    """No description."""
    ...
    def _load_display_options(self):
        try:
            self.show_cut_region = bool(self.fitting_view_model.get_setting('fitting', 'gisaxs_input.show_cut_region', True))
            self.show_center = bool(self.fitting_view_model.get_setting('fitting', 'gisaxs_input.show_center', True))
            cmap = self.fitting_view_model.get_setting('fitting', 'gisaxs_input.colormap', 'viridis')
    ...
    def _save_display_options(self):
        try:
            self.fitting_view_model.set_setting('fitting', 'gisaxs_input.show_cut_region', bool(self.show_cut_region))
            self.fitting_view_model.set_setting('fitting', 'gisaxs_input.show_center', bool(self.show_center))
    ...
    def _get_detector_center_display_coordinates(self):
        try:
            ...
            if shape is None:
                return None
            height, width = shape
            beam_x = float(self.fitting_view_model.get_setting('fitting', 'detector.beam_center_x', width / 2.0))
            beam_y = float(self.fitting_view_model.get_setting('fitting', 'detector.beam_center_y', height / 2.0))
    ...
    def _get_q_axis_extent(self, image_shape):
        ...
        try:
            ...
            pixel_size_x = self.fitting_view_model.get_setting('fitting', 'detector.pixel_size_x', 172.0)  # micrometers
            pixel_size_y = self.fitting_view_model.get_setting('fitting', 'detector.pixel_size_y', 172.0)  # micrometers
            beam_center_x = self.fitting_view_model.get_setting('fitting', 'detector.beam_center_x', width / 2.0)
            beam_center_y = self.fitting_view_model.get_setting('fitting', 'detector.beam_center_y', height / 2.0)
            distance = self.fitting_view_model.get_setting('fitting', 'detector.distance', 2565.0)  # mm
            ...
            _, _, extent = (
                _scientific_commands(self).q_space.axis_labels_and_extent(
                    self._q_detector
                )
            )
            return extent
    ...
    def update_plot(self, x_coords, y_intensity, x_label, y_label, title, log_x=False, log_y=False, normalize=False, y_errors=None):
        ...
        try:
            ...
            if err_data is not None:
                ...
                try:
                    FittingViewBinding._plot_cut_data_with_log_handling(self.ax, x_data, y_data, log_x, markersize=6, linewidth=2)
                except:
    ...
    def _unified_plot_1d_data(self, ax, q, intensity, err, title, log_x, log_y, normalize):
        ...
        try:
            ...
            if err is not None:
                ...
                FittingViewBinding._plot_cut_data_with_log_handling(
...
class AsyncImageLoader(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if file_ext == '.cbf' and not self.fitting_view_model.storage.dependency_available("fabio"):
...
class FittingViewBinding(QObject):
    ...
    def _select_folder_image(self, file_path, frame_index=0):
        try:
            file_path = normalize_path(file_path)
            if not self.fitting_view_model.storage.is_remote_source(file_path) and not os.path.exists(file_path):
    ...
    def _on_points_num_finished(self):
        ...
        # Persist
        try:
            self.preferences.set('fit.points_num', int(n)); self.preferences.save()
        except Exception:
            pass
        was_fitting = self._is_in_fitting_mode() if hasattr(self, '_is_in_fitting_mode') else False
    ...
    def _on_interp_method_changed(self, method: str):
        ...
        try:
            self.preferences.set('fit.interp_method', meth); self.preferences.save()
        except Exception:
            pass
    ...
    def _interpolate_series(self, x, y, x_new, method: str):
        return _scientific_commands(self).cut.interpolate(
            x, y, x_new, method
        )
    ...
    def _restore_gisaxs_input_parameters(self):
        ...
        for widget_name, param_key, default_value in mapping:
            if not hasattr(self.ui, widget_name):
                continue
            try:
                value = self.fitting_view_model.get_setting('fitting', f'gisaxs_input.{param_key}', default_value)
                if value is None:
                    continue
    ...
    def _load_image_display_options(self):
        try:
            settings = QSettings()
            self._show_cut_region = bool(self.fitting_view_model.get_setting('fitting', 'gisaxs_input.show_cut_region', True))
            self._show_center = bool(self.fitting_view_model.get_setting('fitting', 'gisaxs_input.show_center', True))
            cmap = self.fitting_view_model.get_setting('fitting', 'gisaxs_input.colormap', 'viridis')
            ...
            self._threshold_mask_enabled = settings.value(
                ...
                bool(self.fitting_view_model.get_setting('fitting', 'gisaxs_input.threshold_mask_enabled', False)),
                type=bool,
            )
            self._threshold_mask_min = float(settings.value(
                'fitting/gisaxs_input/threshold_mask_min',
                self.fitting_view_model.get_setting('fitting', 'gisaxs_input.threshold_mask_min', -1e12),
            ))
            self._threshold_mask_max = float(settings.value(
                'fitting/gisaxs_input/threshold_mask_max',
                self.fitting_view_model.get_setting('fitting', 'gisaxs_input.threshold_mask_max', 1e12),
            ))
            self._mirror_fill_detector_gaps = settings.value(
                'fitting/gisaxs_input/mirror_fill_detector_gaps',
                bool(self.fitting_view_model.get_setting('fitting', 'gisaxs_input.mirror_fill_detector_gaps', False)),
                type=bool,
            )
            self._mirror_gap_margin_px = int(
                settings.value(
                    'fitting/gisaxs_input/mirror_gap_margin_px',
                    self.fitting_view_model.get_setting('fitting', 'gisaxs_input.mirror_gap_margin_px', 0),
                    type=int,
                )
            )
    ...
    def _save_image_display_options(self):
        try:
            ...
            self.fitting_view_model.set_setting('fitting', 'gisaxs_input.show_cut_region', bool(self._show_cut_region))
            self.fitting_view_model.set_setting('fitting', 'gisaxs_input.show_center', bool(self._show_center))
            self.fitting_view_model.set_setting('fitting', 'gisaxs_input.colormap', self._image_colormap)
            self.fitting_view_model.set_setting('fitting', 'gisaxs_input.flip_ud', bool(self._flip_ud))
    ...
    def _get_mirror_gap_fill_center_x(self):
        try:
            ...
            return float(self.fitting_view_model.get_setting('fitting', 'detector.beam_center_x', width / 2.0))
        except Exception:
            return None
    ...
    def _initialize_ui(self):
        ...
        if hasattr(self.ui, 'gisaxsInputModelCombox'):
            try:
                last_mode = self.fitting_view_model.get_setting('fitting', 'gisaxs_input.load_mode', 'Single')
    ...
    def _get_cached_q_meshgrids(self):
        ...
        try:
            ...
            if hasattr(self, 'current_stack_data') and self.current_stack_data is not None:
                ...
                pixel_size_x = self.fitting_view_model.get_setting('fitting', 'detector.pixel_size_x', 172.0)
                pixel_size_y = self.fitting_view_model.get_setting('fitting', 'detector.pixel_size_y', 172.0)
                beam_center_x = self.fitting_view_model.get_setting('fitting', 'detector.beam_center_x', width / 2.0)
                beam_center_y = self.fitting_view_model.get_setting('fitting', 'detector.beam_center_y', height / 2.0)
    ...
    def _on_import_value_changed(self):
        ...
        try:
            ...
            if not os.path.isabs(file_path_input):
                current_file = self.current_parameters.get('imported_gisaxs_file', '')
                if current_file and (self.fitting_view_model.storage.is_remote_source(current_file) or os.path.exists(current_file)):
                    current_dir = os.path.dirname(current_file)
                    file_path_input = os.path.join(current_dir, file_path_input)
                else:
                    file_path_input = os.path.abspath(file_path_input)

            if not self.fitting_view_model.storage.is_remote_source(file_path_input) and not os.path.exists(file_path_input):
    ...
    def _sync_ui_to_parameters(self):
        ...
        try:
            if hasattr(self.ui, 'gisaxsInputImportButtonValue'):
                ...
                if file_input:
                    if os.path.isabs(file_input) and (self.fitting_view_model.storage.is_remote_source(file_input) or os.path.exists(file_input)):
                        ...
                        if current_file and os.path.dirname(current_file):
                            ...
                            if self.fitting_view_model.storage.is_remote_source(new_path) or os.path.exists(new_path):
                    ...
                    elif os.path.isabs(file_input) and not self.fitting_view_model.storage.is_remote_source(file_input) and not os.path.exists(file_input):
    ...
    def _show_image(self):
        ...
        try:
            ...
            if not self.fitting_view_model.storage.is_remote_source(imported_file) and not os.path.exists(imported_file):
    ...
    def _on_load_mode_changed(self, text: str):
        ...
        try:
            ...
            try:
                self.fitting_view_model.set_setting('fitting', 'gisaxs_input.load_mode', self.load_mode)
            except Exception:
                pass
            self._update_stack_controls_visibility()
    ...
    def _refresh_insitu_workflow_step_styles(self):
        ...
        style_map = {
            ...
            "auto_refine": "color: #16803c;" if self.fitting_view_model.storage.dependency_available("scipy") else "color: #b00020;",
        }
    ...
    def _start_insitu_workflow(self):
        try:
            ...
            if not self.fitting_view_model.storage.is_remote_source(folder) and not os.path.isdir(folder):
            ...
            if self._insitu_workflow_state != "Paused":
                self._insitu_workflow_queue = []
                if self.fitting_view_model.storage.is_remote_source(folder) and normalize_path(folder) not in self._folder_image_scan_cache:
                    self._scan_folder_images_for_file(imported_file)
    ...
    def _start_insitu_sequence_processing(self):
        try:
            ...
            if not folder:
                self._populate_insitu_sequence_folder_default()
                folder = widgets.get("sequence_folder").text().strip() if widgets.get("sequence_folder") else ""
            if not folder or (not self.fitting_view_model.storage.is_remote_source(folder) and not os.path.isdir(folder)):
    ...
    def _auto_find_center(self):
        ...
        try:
            ...
            vertical_profile, horizontal_profile = (
                _scientific_commands(self).image.center_profiles(
    ...
    def _extract_cut_q_mode(self, center_qy, center_qz, height_q, width_q, cut_type: str, points_override: int = None):
        ...
        try:
            ...
            intensity_native, q_native, pixel_indices = (
                _scientific_commands(self).cut.extract_q(
                self.current_stack_data,
                qy_mesh,
                qz_mesh,
                selection,
    ...
    def _extract_cut_pixel_mode(self, center_x, center_y, height, width, cut_type: str, points_override: int = None):
        ...
        try:
            ...
            intensity_native, pixel_coords = (
                _scientific_commands(self).cut.extract_pixel(
                self.current_stack_data,
                selection,
                )
            )
    ...
    def _filter_q_data_for_independent_display(self, q_data, y_data=None):
        """No description."""
        filter_mode = self._get_independent_axis_filter_mode()
        return _scientific_commands(self).curve.filter_for_display(
            q_data, y_data, filter_mode
        )
    ...
    def _valid_y_values_for_limits(self, y_values, log_y=False):
        """No description."""
        try:
            return _scientific_commands(self).curve.valid_y_for_limits(
                y_values, log_y=log_y
            )
        except Exception:
    ...
    def load_fitting_parameters_from_file(self, filepath: str) -> bool:
        ...
        try:
            ...
            if isinstance(model_fitting, dict):
                self.model_params_manager.replace_section(
                    "fitting", copy.deepcopy(model_fitting)
                )
                self.model_params_manager.save_parameters()
    ...
    def _current_ai_profile(self):
        ...
        selected = str(
            settings.get("profile") or _ai_catalog(self).default_profile_name
        )
    ...
    def _ai_constraint_geometries(self) -> list[str]:
        ...
        return sorted(
            {
                _scientific_commands(self).ai.normalize_geometry(item)
    ...
    def _build_manual_refine_setup(self):
        try:
            ...
            model_func = _scientific_commands(self).model.build_function(
                active_shapes
            )
            param_names = _scientific_commands(self).model.parameter_names(
                active_shapes
            )
    ...
    def _default_manual_refine_bounds(self, name: str, value: float):
        return _scientific_commands(self).refinement.default_bounds(
            name, value
        )
