"""Feature-owned binding between the Classification page and ViewModel."""
...
class ClassificationViewBinding(QObject):
    ...
    def _on_import_finished(self, payload, labels: Optional[list[str]]) -> None:
        ...
        self.summary = payload.get(
            "summary"
        ) or self.classification_view_model.validate_dataset(self.samples)
        self._mark_results_outdated()
        self._refresh_everything()
    ...
    def _save_session(self) -> None:
        ...
        if not path:
            return
        try:
            self.classification_view_model.save_session(
                Path(path), self.get_parameters()
            )
    ...
    def _load_session(self) -> None:
        ...
        try:
            self.set_parameters(
                self.classification_view_model.load_session(Path(path))
            )
    ...
    def _export_results_csv(self) -> None:
        ...
        if not path:
            return
        columns = ("rank", "algorithm", "accuracy", "balanced_accuracy", "macro_f1", "weighted_f1", "status", "error")
        rows = tuple(
            (
                rank,
                result.display_name,
