...
from .model_operations import ClassificationModelOperationsMixin
from .state import ClassificationState


class ClassificationViewModel(ClassificationModelOperationsMixin):
    def __init__(
        self,
        *,
        context: AppContext,
        ...
        self._assign_labels = assign_labels
        self._clear_labels = clear_labels
        self._accept_suggestions = accept_suggestions
        self._suggest_clusters = suggest_clusters
    ...
    def validate_dataset(
        ...
        return self._validate_dataset.execute(
            tuple(samples), require_labels=require_labels, allow_mixed=allow_mixed
        )
    def group_samples(self, samples):
        return self._group_samples.execute(tuple(samples))

    def assign_labels(self, samples, sample_ids, label, *, source="manual"):
        return self._assign_labels.execute(
            AssignLabelsRequest(tuple(samples), tuple(sample_ids), label, source)
        )

    def clear_labels(self, samples, sample_ids):
        return self._clear_labels.execute(tuple(samples), tuple(sample_ids))

    def accept_suggestions(self, samples, sample_ids=()):
        return self._accept_suggestions.execute(
            AcceptSuggestedLabelsRequest(tuple(samples), tuple(sample_ids))
        )
