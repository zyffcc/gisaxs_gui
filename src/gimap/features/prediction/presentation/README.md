...
`preview_layout.py` 只重组、渲染 Qt controls，不调用具体 adapter 或科学运行时。Workspace
通过 ViewModel commands 进入 application，不得在 presentation 内复制 workflow。

`views/prediction_page_view.py` 是 Prediction 原始控件的 Python View，
`views/prediction_workspace_view.py` 是最终可见 section hierarchy 的 Python View。
`control_view_factory.py` 只负责实例化 View 和暴露稳定属性，`workspace.py` 只把同一控件实例
装入语义化 host，不得引入第二套静态控件工厂。
