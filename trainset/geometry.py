...
def roi_to_spherical_ranges(config: Dict[str, Any]) -> Dict[str, float]:
    ...
    # Detector x grows from the left ROI edge to the right.  The left extent is
    # negative relative to the direct-beam column and the right extent positive.
    # Keeping the second sign positive is essential when the ROI spans the beam:
    # otherwise a symmetric ROI collapses to phi_min == phi_max.
